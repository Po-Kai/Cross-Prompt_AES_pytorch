...
def get_asap_rep_dataset(test_prompt_id, tokenizer, max_length=512, num_rep=16):
    configs = DataConfigs()
    features_path = f"data/LDA/hand_crafted_final_{test_prompt_id}.csv"
    data_path = os.path.join(configs.DATA_PATH, f"{test_prompt_id}")
    data_configs = {
    ...
    def prepare_dataset(sample):
        ...
        # sample["prompt_input"] = tokenizer(prompt_text, truncation=True, max_length=max_length)
        # sample["essay_input"] = tokenizer(essay_text, rep_text, truncation="only_first", max_length=max_length)
        sample["prompt_input"] = tokenizer(prompt_text, rep_text, truncation="only_first", max_length=max_length)
        sample["essay_input"] = tokenizer(essay_text, truncation=True, max_length=max_length)        
        return sample
    ...
    rep_text = " ".join([f"[unused{i}]" for i in range(num_rep)])
